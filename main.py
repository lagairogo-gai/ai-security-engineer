...
st.sidebar.header("AI Security Assistant")
...
with st.sidebar:
    st.markdown(
        "Welcome to AI Security, an AI-powered Assistant designed to help teams produce better threat models for their applications."
    )
    st.markdown(
        "Threat modelling is a key activity in the software development lifecycle, but is often overlooked or poorly executed. AI Security Assistant aims to help teams produce more comprehensive threat models by leveraging the power of Large Language Models (LLMs) to generate a threat list, attack tree and/or mitigating controls for an application based on the details provided."
    )
    st.markdown("""---""")
...
with st.sidebar:
    st.markdown(
        "Below is an example application description that you can use to test AI Security Assistant:"
    )
    st.markdown(
...
tab1, tab2, tab3 = st.tabs(["Threat Model", "Attack Tree", "Mitigations"])
...
understanding possible vulnerabilities and attack vectors. Use this tab to generate a threat model.
""")
    st.markdown("""---""")
